...
class AprendizViewSet(ModelViewSet):
    ...
    def crear_un_aprendiz_csv(self, request):
        ...
        try:
            lector = list(csv.DictReader(TextIOWrapper(archivo.file, encoding='utf-8-sig'), delimiter=';'))
        except Exception as e:
        ...
        try:
            with transaction.atomic():
                required_fields = ['nom', 'apelli', 'email', 'tipo_dni', 'dni', 'tele', 'gene']
                ...
                try:
                    ...
                    raise ValidationError(f"La ficha con ID {ficha_id} no existe.")

                fecha_naci = fila.get('fecha_naci')
                fecha_naci = datetime.strptime(fecha_naci.strip(), '%d/%m/%Y').date() if fecha_naci else None
                ...
                user = User.objects.create_user(username=username, password=str(dni), email=email)
                ...
                if dni_repre:
                    ...
                    try:
                        ...
                        raise ValidationError(f"Correo de representante inválido: {mail_repre}")
                ...
                ficha.num_apre_pendi_regi = T_apre.objects.filter(ficha=ficha).count()
